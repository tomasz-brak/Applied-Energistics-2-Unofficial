...
public class PartAnnihilationPlane extends PartBaseAnnihilationPlane
    ...
    public void onEntityCollision(final Entity entity) {
        if (this.isAccepting && entity instanceof EntityItem
                && !entity.isDead
                && Platform.isServer()
                    ...
                    if (entity.posX > this.getTile().xCoord && entity.posX < this.getTile().xCoord + 1) {
                        if (entity.posZ > this.getTile().zCoord && entity.posZ < this.getTile().zCoord + 1) {
                            if ((entity.posY > this.getTile().yCoord + 0.9 && this.getSide() == ForgeDirection.UP)
                                    || (entity.posY < this.getTile().yCoord + 0.1
                                            && this.getSide() == ForgeDirection.DOWN)) {
                                capture = true;
                            }
                        }
                    }
                    ...
                    if (entity.posX > this.getTile().xCoord && entity.posX < this.getTile().xCoord + 1) {
                        if (entity.posY > this.getTile().yCoord && entity.posY < this.getTile().yCoord + 1) {
                            if ((entity.posZ > this.getTile().zCoord + 0.9 && this.getSide() == ForgeDirection.SOUTH)
                                    || (entity.posZ < this.getTile().zCoord + 0.1
                                            && this.getSide() == ForgeDirection.NORTH)) {
                                capture = true;
                            }
                        }
                    }
                    ...
                    if (entity.posZ > this.getTile().zCoord && entity.posZ < this.getTile().zCoord + 1) {
                        if (entity.posY > this.getTile().yCoord && entity.posY < this.getTile().yCoord + 1) {
                            if ((entity.posX > this.getTile().xCoord + 0.9 && this.getSide() == ForgeDirection.EAST)
                                    || (entity.posX < this.getTile().xCoord + 0.1
                                            && this.getSide() == ForgeDirection.WEST)) {
                                capture = true;
                            }
                        }
                    }
    ...
    private IAEItemStack storeItemStack(final ItemStack item) {
        ...
        try {
            ...
            final IAEItemStack overflow = Platform
                    .poweredInsert(energy, storage.getItemInventory(), itemToStore, this.mySrc);
                    ...
                    if (hasPower && canStore) {
                        if (modulate) {
                            ...
                            ServerHelper.proxy.sendToAllNearExcept(
                                    ...
                                    64,
                                    w,
                                    new PacketTransitionEffect(x, y, z, this.getSide(), true));
                        } else {
                            this.breaking = true;
    ...
    public TickingRequest getTickingRequest(final IGridNode node) {
        return new TickingRequest(
                TickRates.AnnihilationPlane.getMin(),
                TickRates.AnnihilationPlane.getMax(),
                false,
                true);
    }
    ...
    private boolean canHandleBlock(final WorldServer w, final int x, final int y, final int z) {
        ...
        final boolean ignoreMaterials = material == Material.air || material == Material.lava
                ...
                || material.isLiquid();
        final boolean ignoreBlocks = block == Blocks.bedrock || block == Blocks.end_portal
        ...
        return permissionCache == YesNo.YES && !ignoreMaterials
                && !ignoreBlocks
                && !w.isAirBlock(x, y, z)
                && w.blockExists(x, y, z)
                && w.canMineBlock(player, x, y, z)
                && hardness >= 0f;
    }
     ...
     * Checks if this plane can handle the block at the specific coordinates.
     */
    protected float calculateEnergyUsage(final WorldServer w, final int x, final int y, final int z,
    ...
    private void breakBlockAndStoreItems(final WorldServer w, final int x, final int y, final int z,
            final List<ItemStack> items) {
