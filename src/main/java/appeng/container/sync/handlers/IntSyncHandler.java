...
public class IntSyncHandler extends AbstractSyncHandler {
    ...
    private @Nullable IntChangeListener clientChangeListener;
    private @Nullable IntChangeListener serverChangeListener;

    public IntSyncHandler(final SyncManager manager, final String key, final String fullKey,
     ...
     */
    public void setLocalValue(final int value) {
        this.value = value;
    ...
    public IntSyncHandler onClientChange(final IntChangeListener listener) {
        this.clientChangeListener = listener;
        return this;
    }

    public IntSyncHandler onServerChange(final IntChangeListener listener) {
    ...
    protected void readUpdate(final SyncEndpoint remoteEndpoint, final SyncMode mode, final ByteBuf buf)
        ...
        if (mode != SyncMode.FULL) {
            throw new IOException("IntSyncHandler only supports full updates.");
        }

        final int oldValue = this.value;
        final int newValue = buf.readInt();
        this.value = newValue;
